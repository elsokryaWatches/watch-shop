  ...
  useEffect(() => {
    const fetchWatches = async () => {
      try {
        ...
        const inStockQuery = query(watchesRef, where("stock", ">", 0));
        const querySnapshot = await getDocs(inStockQuery);

        const inStockWatches = querySnapshot.docs.map((doc) => ({
          id: doc.id,
          ...doc.data(),
        }));
          ...
          randomWatches.map(async (watch) => {
            if (watch.images && watch.images.length > 0) {
              ...
              try {
                ...
                console.error(
                  `Error fetching image for ${firstImagePath}`,
                  imageError
                );
